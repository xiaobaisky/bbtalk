...
! function() {
    ...
    Trie.prototype = {
        insert: function(str, pos, idx) {
            ...
            if (pos === str.length) { T.index = idx; return; }
            k = str[pos];
        ...
        build: function(arr) { var len = arr.length; for (var i = 0; i < len; i++) { this.insert(arr[i], 0, i); } },
        searchOne: function(str, pos) {
            ...
            k = str[pos];
            child = T.children[k];
            if (child !== undefined && pos < str.length) { return child.searchOne(str, pos + 1); }
    ...
    if (typeof module !== 'undefined') { module.exports = Trie; } else if (typeof window !== 'undefined') { window._qqWechatEmotionParser.Trie = Trie; }
}();
! function() {
    ...
    function splice(str, index, count, add) { return str.slice(0, index) + add + str.slice(index + count); }

    function keys(map) {
        var list = [];
        for (var k in map) { if (map.hasOwnProperty(k)) list.push(k); }
        return list;
    }
}();